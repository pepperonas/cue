...
- `components/BookmarkButton.tsx` — shared bookmark toggle (mini-btn on cards/list, icon-btn in the detail). Gold fill + highlight when active (`.bookmark-btn.bookmarked`); the icon `motion.span` is keyed by state so it pops on every toggle (reduced-motion → instant).
